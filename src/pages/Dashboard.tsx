...
const features = [
  { title: "AI Tutor", description: "Chat with your personal AI mentor", icon: Bot, path: "/ai-tutor", gradient: "from-blue-500 to-cyan-500" },
  { title: "Code Reviewer", description: "Analyze and improve your code quality", icon: Code2, path: "/code-reviewer", gradient: "from-violet-500 to-purple-500" },
  { title: "Doubt Resolver", description: "Get your questions answered clearly", icon: HelpCircle, path: "/doubt-resolver", gradient: "from-orange-500 to-amber-500" },
  { title: "Road Builder", description: "Get step-by-step learning paths", icon: Map, path: "/road-builder", gradient: "from-emerald-500 to-teal-500" },
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="container mx-auto px-4 py-12">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-12">
          ...
          <h2 className="text-4xl font-bold font-display text-foreground mb-3">Your Learning Dashboard</h2>
          <p className="text-muted-foreground text-lg max-w-md mx-auto">Choose a tool to supercharge your learning journey</p>
        </motion.div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-5xl mx-auto">
          {features.map((feature, i) => (
            <motion.div key={feature.path} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.06 }}>
              <Card className="cursor-pointer shadow-card hover:shadow-card-hover transition-all duration-300 hover:-translate-y-1 border-0 bg-card" onClick={() => navigate(feature.path)}>
                <CardContent className="p-6">
                  <div className={`w-12 h-12 rounded-xl bg-gradient-to-br ${feature.gradient} flex items-center justify-center mb-4`}>
                    <feature.icon className="w-6 h-6 text-primary-foreground" />
                  </div>
                  <h3 className="text-lg font-semibold font-display text-card-foreground mb-1">{feature.title}</h3>
